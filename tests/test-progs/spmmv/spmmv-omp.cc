...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < N; i++) {
        std::cout << "Generating data for the " << i << "th iteration!!!"
                  << std::endl;
        A[i] = &data_A[N * i];
        B[i] = &data_B[N * i];
        C_M[i] = &data_C[N * i];
    ...
    std::cout << "Printed after checkpoint!!!" << std::endl;
...
#pragma omp parallel for
    for (int i = 0; i < N; i++) {
        std::cout << "Running the " << i << "th iteration!!!" << std::endl;
        float lhs = vls[i].a[0];
    ...
    std ::chrono ::duration<double> prog_elapsed =
        std ::chrono ::duration_cast<std ::chrono ::duration<double>>(prog_end -
                                                                      prog_start);
    std ::chrono ::duration<double> mm_elapsed =
        std ::chrono ::duration_cast<std ::chrono ::duration<double>>(mm_end -
                                                                      mm_start);
